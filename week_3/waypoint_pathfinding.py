...
def navigate_to_waypoint(robot_position, wx, wy):
    (x, y, theta) = robot_position
    dx = wx - x
    dy = wy - y
    ...
    angle_to_turn = target_angle - theta

    # Normalize the angle to the range [-pi, pi]
    while angle_to_turn > pi:
        angle_to_turn -= 2 * pi
    while angle_to_turn < -pi:
        angle_to_turn += 2 * pi

    # Calculate the distance to the waypoint
    distance_to_travel = sqrt(dx**2 + dy**2)

    # Rotate to face the waypoint
    rotate(degrees(angle_to_turn), speed=50)
    BP.set_motor_dps(LEFT_MOTOR, 0)
    BP.set_motor_dps(RIGHT_MOTOR, 0)
    time.sleep(2)

    # Drive straight to the waypoint
    drive_straight_for_distance(distance_to_travel, speed=-50)
    BP.set_motor_dps(LEFT_MOTOR, 0)
    BP.set_motor_dps(RIGHT_MOTOR, 0)
    time.sleep(2)

    # Update the robot's position
    new_robot_position = (wx, wy, target_angle)
